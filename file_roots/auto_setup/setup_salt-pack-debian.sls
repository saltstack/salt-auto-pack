...
{%- if base_cfg.build_specific_tag %}
        salt ({{default_branch_version_number_dotted}}+ds-{{release_level}}) stable; urgency=medium
         ...
         -- Salt Project Packaging <saltproject-packaging@vmware.com>  {{apt_date}}
{%- else %}
...
update_versions_debian_{{base_cfg.build_number_uscore}}:
 file.replace:
    - name: {{base_cfg.build_salt_pack_dir}}/file_roots/versions/{{base_cfg.build_number_uscore}}/debian_pkg.sls
